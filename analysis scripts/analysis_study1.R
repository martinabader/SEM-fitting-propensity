...
data.9p.raw <- as.data.frame(fread('fitmeasures_study1_9p.csv'))
data.16p.raw <- as.data.frame(fread('fitmeasures_study1_16p.csv'))
data.30p.raw <- as.data.frame(fread('fitmeasures_study1_30p.csv'))
...
sort(round(apply(LL.9p, 2, logSumExp)))
...
apply(tli.16p, 2, FUN = function(x) sum(x >= .90)/length(x))
...
sort(round(apply(LL.16p, 2, logSumExp)))
...
sort(round(apply(LL.30p, 2, logSumExp)))
