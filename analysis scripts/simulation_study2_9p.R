...
## load sigmas
sigmas <- as.data.frame(fread("sigmas_9p.csv", sep=";"))
...
save.fun <- function(a, b){
  ...
  fwrite(intermediate.res, file = 'fitmeasures_study2_9p.csv', row.names = F, sep = ';')
...
results <- foreach(crep = 1:nreps, .combine = save.fun, .options.snow = opts) %dopar% {
  ...
  tryCatch({
    ...
    res <- matrix(c(crep, N, p, results.cfa1, results.cfa3, results.bif, detS, logDetS), nrow = 1)
    res
